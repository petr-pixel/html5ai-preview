(function(){
  ...
  // FIXED: Preview animation function
  function applyPreviewAnim(){
    ...
    document.querySelectorAll('.banner .headline, .banner .subline, .banner .cta, .banner img').forEach(function(el){
    ...
    var preset = el('animPreset').value;
    var duration = el('animDur').value;
    var loops = Math.min(3, parseInt(el('animLoops').value, 10) || 1);
    
    // Apply new CSS
    style.textContent = presetCss(preset, duration);
    
    // Wait for CSS to be applied, then set iterations
    requestAnimationFrame(function(){
      var elements = document.querySelectorAll('.banner .headline, .banner .subline, .banner .cta, .banner img');
      elements.forEach(function(el){
        el.style.animationIterationCount = String(loops);
      });
      
      // Stop animation after specified time
      var dur = parseFloat(duration) || 2;
      var stopAfter = Math.min(30, loops * (dur + 0.6));
      setTimeout(function(){
        elements.forEach(function(el){
  ...
  // FIXED: Smart extract with API endpoint
  async function smartExtract(rawUrl){
    try{
      if(!rawUrl) return;
      
      // Show loading indicator
      el('importUrl').textContent = 'Načítám...';
      el('importUrl').disabled = true;
      
      // Use API endpoint instead of direct fetch
      var response = await fetch('/api/extract-meta', {
        method: 'POST',
      ...
      if (data.bgUrl) {
        el('bgUrl').value = data.bgUrl;
      }
      if (data.primaryColor) {
        el('cBgColor').value = data.primaryColor;
      }
      if (data.title && !el('headline').value) {
        el('headline').value = data.title.substring(0, 30); // Limit length
      }
      if (data.description && !el('subline').value) {
        el('subline').value = data.description.substring(0, 50);
      }
      ...
      // Fallback - at least set clickUrl
      el('clickUrl').value = rawUrl;
      alert('Extract selhal - použij ručně URL obrázků. Chyba: ' + e.message);
      render();
    } finally {
      // Restore button
      el('importUrl').textContent = 'Extract z URL';
      el('importUrl').disabled = false;
    }
  }
  ...
  function handleFontUpload(input) {
    ...
    // Check file type
    if (!file.name.match(/\.(woff|woff2)$/i)) {
      alert('Pouze WOFF a WOFF2 formáty jsou podporované');
      return;
    }
    
    // Check file size (max 500KB)
    if (file.size > 500 * 1024) {
      alert('Font je příliš velký (max 500KB)');
      return;
    }
    
    var reader = new FileReader();
    reader.onload = function(e) {
      fontDataUrl = e.target.result;
      console.log('Font loaded:', file.name, (file.size / 1024).toFixed(1) + 'KB');
      render();
    };
  ...
  function validate(){
    ...
    var cr = contrastRatio(el('cTextColor').value, el('cBgColor').value);
    if(cr < 3) issues.push({level:'warn',msg:'Nízký kontrast CTA ('+cr.toFixed(2)+')'});
    
    var badge=el('badge');
    var list=el('issues'); list.innerHTML='';
    if(issues.length===0){ 
      badge.className='badge ok'; 
      badge.textContent='OK – připraveno k exportu'; 
    } else { 
      var err=issues.some(i=>i.level==='error'); 
      badge.className='badge '+(err?'err':'warn'); 
      badge.textContent=err?'Problémy – zkontroluj níže':'Varování'; 
      issues.forEach(function(i){ 
        var li=document.createElement('li'); 
        li.textContent='['+i.level.toUpperCase()+'] '+i.msg; 
        list.appendChild(li); 
      }); 
    }
  }
  ...
  on('fontFile','change', function(e){ handleFontUpload(e.target); });
  ...
  on('newProject','click', function(){
    if(confirm('Smazat aktuální projekt a začít znovu?')){
      ...
      // Reset form values
      el('headline').value = 'Teplo do mrazu';
      el('subline').value = 'Funkční vrstvy 24/7';
      el('cta').value = 'Zjistit více';
      el('clickUrl').value = 'https://www.html5ai.cz/';
      el('brandUrl').value = '';
      el('logoUrl').value = '';
      el('bgUrl').value = '';
      el('bg').value = 'linear-gradient(180deg,#17324F,#0A1A2B)';
      
      // Reset sizes to default
      sizes.forEach(function(s, i){
        s.on = DEFAULT_SIZES[i].on;
        var cb = document.querySelector('input[data-size="'+s.key+'"]');
