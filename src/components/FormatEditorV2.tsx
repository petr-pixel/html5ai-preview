...
export function FormatEditorV2({ formatKey, format, sourceImage, onClose, onSave }: FormatEditorV2Props) {
  ...
  // Načíst uložený textLayer nebo vytvořit nový
  const [textLayer, setTextLayer] = useState<TextLayer>(() => {
    const saved = perFormatTextLayers[formatKey]
    if (saved) {
      return saved as TextLayer
    }
    return createDefaultTextLayer(format, { headline: textOverlay.headline, subheadline: textOverlay.subheadline, cta: textOverlay.cta, ctaColor: textOverlay.ctaColor }, currentBrandKit)
  })
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selectedElement && selectedElement !== 'image' && !editingElement) {
        ...
        setSelectedElement(null) // Reset výběru po skrytí
        saveToHistory()
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Uložit pozici obrázku
      setFormatOffset(formatKey, imageOffset)
      // Uložit textLayer pro příští otevření
  ...
  // selectedConfig - pouze pokud je element viditelný
  const selectedConfig = useMemo(() => {
    if (!selectedElement || selectedElement === 'image') return null
    if (selectedElement === 'logo') {
      return textLayer.logo.visible ? { type: 'logo' as const, element: textLayer.logo } : null
    }
    const el = textLayer[selectedElement]
    return el.visible ? { type: selectedElement, element: el } : null
              ...
              <div className="space-y-2">
                {(['headline', 'subheadline', 'cta'] as const).map((type) => (
                  <button key={type} onClick={() => { 
                    const newVisible = !textLayer[type].visible
                    updateElement(type, { visible: newVisible })
                    // Pokud skrýváme vybraný element, zrušit výběr
                    if (!newVisible && selectedElement === type) setSelectedElement(null)
                    saveToHistory()
                  }} className={cn('w-full flex items-center justify-between px-3 py-2 rounded-lg border text-left', textLayer[type].visible ? type === 'headline' ? 'bg-blue-50 border-blue-200 text-blue-700' : type === 'subheadline' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-gray-50 border-gray-200 text-gray-400')}>
                    <span className="text-sm font-medium capitalize">{type}</span>
                    {textLayer[type].visible ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                  </button>
                ))}
                <button onClick={() => { 
                  const newVisible = !textLayer.logo.visible
                  updateLogo({ visible: newVisible })
                  if (!newVisible && selectedElement === 'logo') setSelectedElement(null)
                  saveToHistory()
                }} disabled={!currentBrandKit?.logoMain} className={cn('w-full flex items-center justify-between px-3 py-2 rounded-lg border text-left', textLayer.logo.visible ? 'bg-purple-50 border-purple-200 text-purple-700' : 'bg-gray-50 border-gray-200 text-gray-400', !currentBrandKit?.logoMain && 'opacity-50')}>
