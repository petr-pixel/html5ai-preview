...
interface Props {
  ...
  onClose: () => void
  onSave?: (textLayer: any, renderedImage: string) => void
}
...
export function FormatEditorV2({ formatKey, format, sourceImage, onClose, onSave }: Props) {
  ...
  const handleOutpaint = useCallback(async () => {
    ...
    if (!sourceImage) {
      toast.error('Chybí zdrojový obrázek')
      return
    }
    ...
    try {
      ...
      if (result.success && result.image) {
        ...
        if (result.usedFallback) {
          toast.success('Pozadí dopočítáno (blur)', { description: 'Pro AI outpainting nastavte OpenAI klíč' })
        } else {
          toast.success(`Pozadí dopočítáno pomocí AI`)
        }
      } else {
        toast.error('Nepodařilo se dopočítat pozadí', { description: result.error })
      }
    } catch (err: any) {
      console.error('Outpaint error:', err)
      toast.error('Chyba při dopočítávání', { description: err.message })
    } finally {
