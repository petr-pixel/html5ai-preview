...
export function SourceVariantsPanel() {
  ...
  const handleGenerateVariants = async () => {
    if (!prompt.trim()) {
      toast.warning('Nejprve zadejte prompt')
      return
    }
    if (!apiKeys.openai) {
      toast.error('Nejprve nastavte OpenAI API klíč', {
        description: 'Přejděte do Settings a zadejte váš API klíč',
      })
      return
    }
    ...
    try {
      ...
      if (result.success && result.images.length > 0) {
        ...
        toast.success(`Vygenerováno ${newVariants.length} variant`)
      } else {
      ...
      toast.error('Generování selhalo', { description: error.message })
    } finally {
