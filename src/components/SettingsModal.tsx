  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (result.valid) {
        ...
        setTestMessage(result.error || 'Připojení úspěšné') // error může obsahovat warning
      } else {
        setTestStatus('error')
        setTestMessage(result.error || 'Připojení selhalo')
      }
    } catch (err) {
      setTestStatus('error')
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-white/80">AI Outpainting</span>
              </div>
              <p className="text-xs text-white/50 mb-2">
                Pro dopočítání pozadí se automaticky použije DALL-E 2 z vašeho OpenAI účtu.
              </p>
              <p className="text-xs text-emerald-400/80">
                💡 Bez klíče se použije blur fallback (rozmazané pozadí) - zdarma.
              </p>
            </div>

            {/* Pricing Info */}
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <h4 className="text-sm font-medium text-white/80 mb-3">Orientační ceny OpenAI</h4>
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-white/50">DALL-E 2 outpaint:</span>
                  <span className="text-white/80">$0.02/obrázek</span>
                </div>
