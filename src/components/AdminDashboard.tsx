...
export function AdminDashboard() {
  ...
  const updateUserPlan = async (userId: string, newPlan: 'free' | 'pro' | 'enterprise') => {
    ...
    try {
      // Limity podle Supabase Free tier
      const storageLimits = {
      ...
      const { error } = await supabase
        ...
        .update({ 
          plan: newPlan,
          storage_limit: storageLimits[newPlan],
      ...
      setUsers(users.map(u => 
        u.id === userId ? { ...u, plan: newPlan, storage_limit: storageLimits[newPlan] } : u
      ))
      setEditingUser(null)
    } catch (err: any) {
  ...
  const toggleUserAdmin = async (userId: string, isAdmin: boolean) => {
    setSavingUser(userId)
    try {
      // Admin automaticky dostává enterprise plán
      const updates: any = { 
        is_admin: isAdmin,
        updated_at: new Date().toISOString()
      }
      ...
      if (isAdmin) {
        ...
        updates.storage_limit = 500 * 1024 * 1024 // 500 MB
      }
      ...
      setUsers(users.map(u => 
        u.id === userId ? { 
          ...u, 
          is_admin: isAdmin,
          ...(isAdmin ? { plan: 'enterprise' as const, storage_limit: 500 * 1024 * 1024 } : {})
        } : u
      ))
    } catch (err: any) {
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              ...
              Storage limity podle plánu
            </h2>
            <div className="grid grid-cols-3 gap-4">
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                <PlanBadge plan="free" />
                <div className="text-2xl font-bold text-white mt-3">50 MB</div>
              </div>
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                <PlanBadge plan="pro" />
                <div className="text-2xl font-bold text-white mt-3">200 MB</div>
              </div>
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                <PlanBadge plan="enterprise" />
                <div className="text-2xl font-bold text-white mt-3">500 MB</div>
              </div>
            </div>
          </div>
