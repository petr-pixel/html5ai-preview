 ...
 * Vždy vrací kladnou cenu
 */
export function formatPrice(price: number): string {
  // Ujisti se že cena je kladná a validní
  const safePrice = Math.abs(price || 0)
  
  if (!isFinite(safePrice) || safePrice === 0) return '$0.00'
  if (safePrice < 0.01) return '< $0.01'
  if (safePrice < 0.1) return `$${safePrice.toFixed(3)}`
...
export function CostBadge({ cost, className }: CostBadgeProps) {
  // Nikdy nezobrazuj záporné nebo nulové ceny
  if (!cost || cost <= 0 || !isFinite(cost)) return null
  
  // Ujisti se že cena je kladná
  const safeCost = Math.abs(cost)
