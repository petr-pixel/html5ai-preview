...
function SidebarItem({ icon, label, active, onClick, badge, pro }: SidebarItemProps) {
  return (
    <button
      onClick={onClick}
...
export function UnifiedSidebar({
  ...
  return (
    ...
    >
            ...
            <div key={platformId} className="mb-1">
              ...
              {selectedPlatform === platformId && (
                <motion.div 
                  ...
                  className="ml-5 mt-1 space-y-0.5 border-l border-white/10 pl-3"
                >
      ...
      {/* Pro Banner */}
      <div className="relative m-3 p-4 rounded-2xl overflow-hidden">
        {/* Background gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-violet-600/30 via-purple-600/20 to-cyan-600/30" />
        <div className="absolute inset-0 backdrop-blur-sm" />
        <div className="absolute inset-0 border border-violet-500/30 rounded-2xl" />
        
        {/* Glow effect */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-8 bg-violet-500/30 blur-2xl" />
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-bold text-white">Upgrade na Pro</span>
          </div>
          <p className="text-xs text-white/60 mb-3 leading-relaxed">
            Neomezené generování, všechny formáty, prioritní podpora
          </p>
          <button className="w-full py-2.5 rounded-xl font-semibold text-sm text-white transition-all duration-300 flex items-center justify-center gap-2 group"
            style={{
              background: 'linear-gradient(135deg, #8B5CF6 0%, #06B6D4 100%)',
              boxShadow: '0 4px 15px -3px rgba(139, 92, 246, 0.4)',
            }}
          >
            <span>499 Kč/měsíc</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {stats.map((stat, index) => (
          <motion.div 
            ...
            className={`glass-card-hover p-6 ${stat.glow}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="space-y-3">
            <button className="w-full flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-violet-500/30 transition-all duration-200 text-left group">
            ...
            <button className="w-full flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-cyan-500/30 transition-all duration-200 text-left group">
            ...
            <button className="w-full flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-emerald-500/30 transition-all duration-200 text-left group">
