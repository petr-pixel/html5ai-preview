...
// ============================================================================
// OUTPAINT IMAGE (Wrapper for MagicResize compatibility)
// ============================================================================
...
/**
 * Outpaint image to target dimensions
 * Uses DALL-E 2 edit API under the hood
 */
export async function outpaintImage(
  config: OpenAIConfig,
  params: OutpaintParams
): Promise<OutpaintResult> {
  const timerId = perfStart('outpaintImage')
  
  debug.api('OpenAI', 'outpaintImage', {
  ...
  try {
    ...
    // Calculate scaling to cover (will be cropped)
    const scale = Math.max(
      params.targetWidth / srcWidth,
      params.targetHeight / srcHeight
    )
    
    // Calculate centered position
    const scaledWidth = srcWidth * scale
    const scaledHeight = srcHeight * scale
    const offsetX = (params.targetWidth - scaledWidth) / 2
    const offsetY = (params.targetHeight - scaledHeight) / 2
    
    // Fill with transparent (for PNG with alpha)
    ctx.clearRect(0, 0, params.targetWidth, params.targetHeight)
    
    // Draw source image centered
    ctx.drawImage(img, offsetX, offsetY, scaledWidth, scaledHeight)
    
    // Check if we actually need outpainting (any transparent areas)
    ...
    if (!hasTransparent) {
      // No outpainting needed, just return the resized image
      const result = canvas.toDataURL('image/png')
      perfEnd(timerId)
      return { success: true, image: result, cost: 0 }
    }
    
    // Convert canvas to PNG blob for DALL-E 2 edit API
    const pngBlob = await new Promise<Blob>((resolve) => {
      canvas.toBlob((blob) => resolve(blob!), 'image/png')
    })
    
    // Call edit API
    const editResult = await editImage(config, {
      image: pngBlob,
      prompt: params.prompt || 'Continue the background seamlessly, matching the existing style, colors, and lighting',
      size: '1024x1024', // DALL-E 2 only supports square
    })
    
    if (!editResult.success || !editResult.image) {
      perfEnd(timerId)
      return { success: false, error: editResult.error }
    }
    
    // The result is 1024x1024, resize to target dimensions
    const resultImg = await loadImageElement(editResult.image)
    ...
    // Scale result to target size
    finalCtx.drawImage(resultImg, 0, 0, params.targetWidth, params.targetHeight)
    
    const finalImage = finalCanvas.toDataURL('image/png')
    
    perfEnd(timerId)
    debug.success('OpenAI', 'outpaintImage completed')
    ...
    perfEnd(timerId)
    debug.error('OpenAI', 'outpaintImage failed', error)
    return { success: false, error: error.message }
  }
}
