/**
 * Outpainting Module v3.1 - OpenAI DALL-E 2
 * 
...
export async function outpaintWithOffset(params: OutpaintParams): Promise<OutpaintResult> {
  ...
  try {
    ...
    console.log('[Outpaint] Source:', srcImg.width, 'x', srcImg.height)
    console.log('[Outpaint] Target:', targetWidth, 'x', targetHeight)
    console.log('[Outpaint] Offset:', offsetX, '%,', offsetY, '%')
    ...
    console.log('[Outpaint] Empty areas:', {
    ...
    if (!placement.needsOutpainting) {
      console.log('[Outpaint] No empty areas - just cropping')
      return renderFinal(srcImg, targetWidth, targetHeight, placement)
    }
    
    // 4. Zkusit DALL-E 2 (pokud máme klíč)
    if (openaiKey) {
      console.log('[Outpaint] Trying DALL-E 2...')
      const result = await dalleOutpaint(srcImg, targetWidth, targetHeight, placement, openaiKey)
      if (result.success) {
        console.log('[Outpaint] DALL-E 2 success!')
        return result
      }
      console.warn('[Outpaint] DALL-E 2 failed:', result.error)
    } else {
      console.log('[Outpaint] No API key provided')
    }
    
    // 5. Fallback na blur
    console.log('[Outpaint] Using blur fallback')
...
async function dalleOutpaint(
  ...
  try {
    ...
    console.log('[DALL-E] Scaled target:', scaledTargetW, 'x', scaledTargetH)
    console.log('[DALL-E] Image rect:', imgX, imgY, imgW, imgH)
    ...
    console.log('[DALL-E] PNG size:', (pngBlob.size / 1024).toFixed(1), 'KB')
